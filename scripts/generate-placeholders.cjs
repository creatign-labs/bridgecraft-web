/**
 * Generate minimal placeholder images for testing the upload pipeline.
 * Creates valid JPEG and PNG files using raw binary data (no dependencies).
 */

const fs = require("fs");
const path = require("path");

const dir = path.resolve(__dirname, "../images");
fs.mkdirSync(dir, { recursive: true });

// Minimal valid JPEG (1x1 pixel, gray)
const JPEG_BYTES = Buffer.from([
  ,
]);

// Minimal valid PNG (1x1 pixel, blue #7bfbfc for icons)
const PNG_BYTES = Buffer.from(
  "89504e470d0a1a0a0000000d49484452000000010000000108020000009001" +
    "2e00000000174944415408d76360f8cf80004c2c18000000060001e53e4438" +
    "0000000049454e44ae426082",
  "hex",
);

const jpgFiles = [
  "hero-homepage.jpg",
  "hero-about-intro.jpg",
  "hero-about-vision.jpg",
  "hero-about-mission.jpg",
  "hero-about-strategy.jpg",
  "hero-about-team.jpg",
  "hero-services.jpg",
  "hero-projects.jpg",
  "hero-sectors.jpg",
  "hero-clients.jpg",
  "hero-careers.jpg",
  "hero-contact.jpg",
  "hero-brochure.jpg",
  "service-preconstruction.jpg",
  "service-structural.jpg",
  "service-geotechnical.jpg",
  "service-geophysical.jpg",
  "project-marine-bridge-nh04.jpg",
  "project-deep-geotech-middle-strait.jpg",
  "project-nh04-corridor-26km.jpg",
  "project-nh45c-60km.jpg",
  "project-thiruvarur-bypass.jpg",
  "project-rob-nh67.jpg",
  "project-manair-railway.jpg",
  "project-solar-350mw-phase1.jpg",
  "project-solar-350mw-phase2.jpg",
  "project-cpwd-gri-dindigul.jpg",
];

const pngFiles = [
  "icon-preconstruction.png",
  "icon-structural.png",
  "icon-geotechnical.png",
  "icon-geophysical.png",
];

for (const f of jpgFiles) {
  fs.writeFileSync(path.join(dir, f), JPEG_BYTES);
}
for (const f of pngFiles) {
  fs.writeFileSync(path.join(dir, f), PNG_BYTES);
}

console.log(`Created ${jpgFiles.length + pngFiles.length} placeholder images in ${dir}`);
